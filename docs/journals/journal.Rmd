...
* MZ will look for some examples from research project funding applications.
* DMP should include clear rules on missing values
* DMP should clearly and comprehensively describe the migration process and any issues that arose during it and any decisions that had to be made 
...
* Additionally there is the question of what does 'first use' even mean? This should be defined somewhere clearly.
...
It would be good to have a clearer understanding of the local partner's process of data input. What are they doing now for Chase Africa as well as what they are doing for others e.g. their government. This is to get a better idea of how much effort they are already putting into this for CA's sake, whether they find this useful for their own record keeping. 
...
* a new table - i.e. a tidy table with no subtotals and no calculations in the spreadsheet. this would be better data managgement practice, would not significantly change the amount of effort required on their part, and would make the data migration safer and more streamlined. 
...
Current proposal that allows for all data collection options described above is to have a google sheet with separate worksheets for 
...
* new inputs - if data continues to be provided via an excel table (current or new) this is where CP inputs it into a separate sheet, which is not protected from input.
* main consolidated data - this spreadsheet takes data from all three sheets and merges it into one, additionally this is where any calculations are performed, not before. Again this is a spreadsheet that should be protected, as once it is set up there is no need for further intervention. 

### Dashboard

Google Data Studio provides a great platform for setting up a dashboard which would allow:
