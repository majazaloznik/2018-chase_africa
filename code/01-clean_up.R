...
dandelion_2015%>%
  ...
  select(-del) %>%
  mutate(fp_st_pills_1mth =ifelse(is.na(fp_st_pills_1mth), 0, fp_st_pills_1mth) ) %>%
  mutate(fund_date= ifelse(is.na(fund_date), as.Date("2015-09-14"), fund_date)) %>%
  mutate(fund_date =as.Date(fund_date,origin = "1970-01-01")) -> dandelion_2015
...
dandelion_2016 %>%
  ...
  mutate(fund_gbp = as.numeric(fund_gbp), fund_ksh = as.numeric(fund_ksh)) %>%
  select(-del)  %>%
  mutate(fund_date= ifelse(is.na(fund_date), as.Date("2016-09-01"), fund_date)) %>%
  mutate(fund_date =as.Date(fund_date,origin = "1970-01-01")) ->  dandelion_2016
